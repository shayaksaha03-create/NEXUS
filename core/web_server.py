...
class NexusWeb:
    ...
    def _register_routes(self):
        ...
        def get_stats():
            ...
            try:
                ...
                try:
                    ...
                    if evo_eng:
                        ...
                        if hasattr(evo_eng, 'get_proposals'):
                            ...
                            props_array = evo_eng.proposals or []
                            for i, p in enumerate(props_array):
                                if i >= 20: break
                                if isinstance(p, dict):
                                    evo_deep["proposals"].append({"name": p.get("name", p.get("title", "?")), "priority": p.get("priority", "medium"), "status": p.get("status", "pending"), "date": str(p.get("created", p.get("date", "")))[:16]})
                        # History
                        if hasattr(evo_eng, 'get_history'):
                            ...
                            hist_arr = evo_eng.history or []
                            keep = max(0, len(hist_arr) - 10)
                ...
                return jsonify({
                    ...
                    "context_stats": context_stats_data,
        ...
        def send_message():
            ...
            try:
                ...
                thread = threading.Thread(
                    ...
                    args=(task_id, user_input, user["user_id"], user["username"], images_data),
                    daemon=True
                )
                thread.start()
        ...
        @self.app.route("/api/immune")
        def get_immune_status():
            """Get immune system status"""
            try:
                if hasattr(self.brain, '_immune_system') and self.brain._immune_system:
                    stats = self.brain._immune_system.get_stats()
                    return jsonify(stats)
                return jsonify({"running": False, "error": "Immune system not loaded"})
            except Exception as e:
                logger.error(f"Get immune status error: {e}")
                return jsonify({"error": str(e)}), 500

        @self.app.route("/api/immune/toggle", methods=["POST"])
        def toggle_immune_system():
            """Enable or disable the immune system"""
            user = self._require_auth()
            if not user:
                return jsonify({"error": "Authentication required"}), 401
            ...
            try:
                ...
                if enable is None:
                    ...
                    enable = not getattr(immune_system, '_running', False)

                if enable:
                    if not getattr(immune_system, '_running', False):
                        immune_system.start()
                    return jsonify({"success": True, "running": True, "message": "Immune system enabled"})
                else:
                    if getattr(immune_system, '_running', False):
                        immune_system.stop()
                    return jsonify({"success": True, "running": False, "message": "Immune system disabled"})
            except Exception as e:
                logger.error(f"Toggle immune system error: {e}")
                return jsonify({"success": False, "error": str(e)}), 500
